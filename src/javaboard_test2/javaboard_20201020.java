...
public class javaboard_20201020 {
	public static void main(String[] args) {
		...
		ArrayList<Article> articles = new ArrayList<>(); // Article 클래스 객체 추가
		...
		while(true) { //무한 반복 루프
			...
			if(cmd.equals("add")) { //게시물 추가 코드 시작
				...
				Article a = new Article(); // 아티클 클래스 추가
				
				a.id = no; //아티클 클래스 에 있는 id 이용
				...
				a.title = title; // 아티클 클래스 에 있는 title 사용 
				titles.add(title);
				System.out.println("게시물 내용을 입력해주세요 :");
				String body = sc.next();
				a.body = body; //아티클 클래스 에 있는 body 사용
				bodies.add(body); // 게시물 추가 끝
				
				articles.add(a); //아티클 추가
